...
<div id="importBox" class="hide">
    <form id="importForm" action="${ctx}/surgicalconsentbook/preOperativeConsent/export?type=dj" method="post"
...
<ul class="nav nav-tabs">
    ...
    <c:if test="${node ne 'sjy' and node ne 'fpy'}">
        <shiro:hasPermission name="complaint:complaintInfo:edit">
            <li><a href="${ctx}/complaint/complaintInfo/form">投诉接待添加</a></li>
        </shiro:hasPermission>
    </c:if>
</ul>
<form:form id="searchForm" modelAttribute="complaintInfo" action="${ctx}/complaint/complaintInfo/?node=${node}"
    ...
    <ul class="ul-form">
        ...
        <c:if test="${fns:getUser().company.officeType eq '1'}">
            <li><label>涉及医院：</label>
                <sys:treeselect id="involveHospital" name="involveHospital" value="${complaintInfo.involveHospital}"
                                labelName="hospitalName" labelValue="${complaintInfo.hospitalName}"
                                title="部门" url="/sys/office/treeData?type=1&officeType=2" isAll="true"
                                cssClass="input-small" allowClear="true" notAllowSelectParent="false"/>
            </li>
        </c:if>
        <li><label>涉及科室：</label>
            <sys:treeselect id="involveDepartment" name="involveDepartment" value="${complaintInfo.involveDepartment}"
                            labelName="testTree"
        ...
        <li><label>接待日期：</label>
            ...
            　--　
            <input id="receptionEndDate" name="receptionEndDate" type="text" readonly="readonly" maxlength="20"
...
<table id="contentTable" class="table table-bordered table-condensed">
    ...
    <c:forEach items="${page.list}" var="complaintInfo">
        <c:choose>
            <c:when test="${complaintInfo.nameCount>=2||complaintInfo.nameCount!=1}">
                <tr style="background: rgba(255,0,0,0.6)">
            </c:when>
            <c:otherwise>
        ...
        <shiro:hasPermission name="complaint:complaintInfo:edit">
            <td>
                <c:if test="${(complaintInfo.complaintMain.act.taskDefKey eq 'enrollment' and complaintInfo.complaintMain.act.assigneeName eq fns:getUser().loginName) or (complaintInfo.createBy.id eq fns:getUser().id and empty complaintInfo.complaintMain.procInsId)}">
                    ...
                    <a href="${ctx}/complaint/complaintInfo/delete?id=${complaintInfo.complaintId}"
                       onclick="return confirmx('确认要删除该投诉接待吗？', this.href)">删除</a>
                </c:if>
