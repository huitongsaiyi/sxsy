...
<body>
	...
	<form:form id="searchForm" modelAttribute="user" action="${ctx}/sys/user/list" method="post" class="breadcrumb form-search ">
        ...
        <input type="hidden" id="company"name="company" value="${user.office.officeType}"/>
		...
		<ul class="ul-form">
			<li><label>归属公司：</label><sys:treeselect id="company" name="company.id" value="${user.company.id}" labelName="company.name" labelValue="${user.company.name}"
				title="公司" url="/sys/office/treeData?type=1&officeType=${user.office.officeType}" cssClass="input-small" allowClear="true" isAll="true"/></li>
			<li><label>登录名：</label><form:input path="loginName" htmlEscape="false" maxlength="50" class="input-medium"/></li>
			<li class="clearfix"></li>
			<li><label>归属部门：</label><sys:treeselect id="office" name="office.id" value="${user.office.id}" labelName="office.name" labelValue="${user.office.name}" 
				title="部门" url="/sys/office/treeData?type=2&&officeType=${user.office.officeType}" cssClass="input-small" allowClear="true" notAllowSelectParent="true"/></li>
		...
		<c:forEach items="${page.list}" var="uu" varStatus="xuhao">
			<tr>
				...
				<shiro:hasPermission name="sys:user:edit"><td>
    				<a href="${ctx}/sys/user/form?id=${uu.id}&officeType=${user.office.officeType}&sh=${user.auditing}">审核</a>
